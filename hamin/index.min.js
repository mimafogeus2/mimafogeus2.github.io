...
var getRandom = function getRandom(max, min) {
  return Math.floor(Math.random() * (max - (min || 0) + 1)) + (min || 0);
};

var numOf = function numOf(options, num) {
  return Array.from({
    length: num
  }).map(function () {
    return options[getRandom(options.length - 1)];
  });
};
...
var PATTERNS = [
  ...
  function (_ref2) {
    var VEGETARIAN = _ref2.VEGETARIAN,
      VEGAN = _ref2.VEGAN,
      LENTILS = _ref2.LENTILS,
      NUTS = _ref2.NUTS,
      VEGETABLES = _ref2.VEGETABLES,
      LEAVES = _ref2.LEAVES,
      SPICES = _ref2.SPICES,
      SAUCES = _ref2.SAUCES,
      EXTRAS = _ref2.EXTRAS;
  ...
  function (_ref3) {
    var MEAT = _ref3.MEAT,
      FISH = _ref3.FISH,
      LENTILS = _ref3.LENTILS,
      NUTS = _ref3.NUTS,
      VEGETABLES = _ref3.VEGETABLES,
      LEAVES = _ref3.LEAVES,
      SPICES = _ref3.SPICES,
      SAUCES = _ref3.SAUCES,
      EXTRAS = _ref3.EXTRAS;
...
var TITLE_PATTERNS = [
  function () {
    return ["החמין"].concat(
      getRandom(3)
        ? []
        : ["\u05D4" + LOCATIONS_ADJ[getRandom(LOCATIONS_ADJ.length - 1)]],
      ["של", NAMES[getRandom(NAMES.length - 1)]],
      getRandom(2)
        ? []
        : ["\u05DE" + KNOWN_FROM[getRandom(KNOWN_FROM.length - 1)]],
  ...
  function () {
    return [
      "החמין של",
      FAMILY_RELATIONS[getRandom(FAMILY_RELATIONS.length - 1)]
    ].concat(getRandom(8) ? [] : ["(דרסה אותה משאית)"]);
  },
  function () {
    return ["חמין נוסח", LOCATIONS[getRandom(LOCATIONS.length - 1)]];
  }
];

var makeKnapsack = function makeKnapsack(source) {
  return [].concat(source).sort(function () {
    return Math.random() - 0.5;
  });
};

var makeKnapsacks = function makeKnapsacks() {
  return Object.fromEntries(
    Object.entries(INGREDIENTS).map(function (_ref4) {
      var key = _ref4[0],
        val = _ref4[1];
      return [key, makeKnapsack(val)];
    })
  );
};

var getIngredient = function getIngredient(_ref5) {
  var name = _ref5[0],
    fraction = _ref5[1],
    max = _ref5[2],
    singularUnit = _ref5[3],
    pluralUnit = _ref5[4];
  var maxRand = max / fraction - 1;
  var amount = Math.floor(Math.random() * maxRand) + 1;
  var amountInFractions = amount * fraction;
  var unit = amountInFractions > 1 && pluralUnit ? pluralUnit : singularUnit;
  return amountInFractions + " " + unit + " " + name;
};

var getIngredientFromCategory = function getIngredientFromCategory(category) {
  return getIngredient(category.pop());
};

var getIngredientList = function getIngredientList() {
  return PATTERNS[getRandom(PATTERNS.length - 1)](makeKnapsacks())
    .map(getIngredientFromCategory)
    .join("\n");
};

var getRecipeName = function getRecipeName() {
  return TITLE_PATTERNS[getRandom(TITLE_PATTERNS.length - 1)]().join(" ");
};

var getInstructions = function getInstructions() {
  return INSTRUCTIONS[getRandom(INSTRUCTIONS.length - 1)];
};
